...
/*
        ...
        //Smith Waterman
        for(int i = 1; i < n + 1; i++){ S[0][i] = 0; T[0][i] = Traceback::HOME;} // erste Zeile
        for(int i = 1; i < m + 1; i++){ S[i][0] = 0; T[i][0] = Traceback::HOME;} // erste Spalte
        ...
        vector<int> best = {-1, -1, -1}; //bestes lokale Alignment
        for(int i = 1; i < m + 1; i++){
            for (int j = 1; j < n + 1; j++){
        ...
        while(temp != Traceback::HOME && S[a][b] > 0){
            temp = T[a][b];
            cout << S[a][b] << endl;     
...
int main()
{
    ...
    std::cout << "(if code is correctly pushed into Git && alignment_main.cpp is correct)\n";
